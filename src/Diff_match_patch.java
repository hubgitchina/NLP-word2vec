...
public class Diff_match_patch {
		...
		for (int d = 0; d < max_d; d++) {
			...
			if (doubleEnd) {
				...
				for (int k = -d; k <= d; k += 2) {
					...
					while (!done && x < text1_length && y < text2_length && text1
							.charAt(text1_length - x - 1) == text2.charAt(text2_length - y - 1)) {
						x++;
						y++;
						footstep = diff_footprint(text1_length - x, text2_length - y);
						if (!front && (footsteps.containsKey(footstep))) {
		...
		for (int d = v_map.size() - 2; d >= 0; d--) {
			while (true) {
				if (v_map.get(d).contains(diff_footprint(x - 1, y))) {
					...
					x--;
					y--;
					assert (text1.charAt(x) == text2
							.charAt(y)) : "No diagonal.  Can't happen. (diff_path1)";
		...
		for (int d = v_map.size() - 2; d >= 0; d--) {
			while (true) {
				if (v_map.get(d).contains(diff_footprint(x - 1, y))) {
					...
					x--;
					y--;
					assert (text1.charAt(text1.length() - x - 1) == text2.charAt(
							text2.length() - y - 1)) : "No diagonal.  Can't happen. (diff_path2)";
					if (last_op == Operation.EQUAL) {
						path.getLast().text += text1.charAt(text1.length() - x - 1);
					} else {
	 ...
	 * falls on logical boundaries. Scores range from 5 (best) to 0 (worst).
	 * 
	 * @param one
	 *            First string.
	 * @param two
	...
	public String diff_prettyHtml(LinkedList<Diff> diffs) {
		...
		for (Diff aDiff : diffs) {
			...
			case INSERT:
				html.append("<INS STYLE=\"background:green;\" TITLE=\"i=").append(i).append("\">")
						.append(text).append("</INS>");
				break;
			case DELETE:
				html.append("<DEL STYLE=\"background:red;\" TITLE=\"i=").append(i).append("\">")
	...
	public static void main(String[] args) {
		...
		String text1 = "我方在“福市财政资金补助项目管理系统”的项目实施过程，为保证该项目的工程进度、zhiliang，符合数办项目的实施管理规范与要求。";
		String text2 = "我方在“福州市财政资金补助项目管理系统”的项目实施过程，为保证该项目的工程进度、质量，符合目的实施管理规范与要求。";
